...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr):
    ...
    return np.mean(acc), np.mean(val)
    ...
    for lr in learning_rates:
        for epoch in training_epoch:
            pretrained_model, train_dataloader, validation_dataloader, test_dataloader = pre_process(args.model,
                                                                                             ...
                                                                                             args.device,
                                                                                             args.small_subset)
            _,valacc = train(pretrained_model, epoch, train_dataloader, validation_dataloader, "cuda", lr)
            valrank.append([valacc,lr,epoch])

    maxval = np.amax(valrank,axis=0)[0]
    index = np.where(maxval==valrank)[0][0]
